    ...
    public void onInitialize() {
        ...
        LootTableEvents.MODIFY.register(((resourceManager, manager, id, supplier, setter) -> {
            if(LootTables.END_CITY_TREASURE_CHEST.equals(id) ||
                ...
                supplier.pools(Arrays.stream(manager.getTable(INDEXED_LOOT).pools).toList());
        ...
        LootTableEvents.MODIFY.register(((resourceManager, manager, id, supplier, setter) -> {
            if(LootTables.NETHER_BRIDGE_CHEST.equals(id)) {
                supplier.pools(Arrays.stream(manager.getTable(INDEXED_NETHER_BRIDGE).pools).toList());
            }
        }));

        LootTableEvents.MODIFY.register(((resourceManager, manager, id, supplier, setter) -> {
            if(LootTables.PILLAGER_OUTPOST_CHEST.equals(id)) {
                supplier.pools(Arrays.stream(manager.getTable(INDEXED_OUTPOST).pools).toList());
            }
        }));

        LootTableEvents.MODIFY.register(((resourceManager, manager, id, supplier, setter) -> {
            if(LootTables.WOODLAND_MANSION_CHEST.equals(id)) {
                supplier.pools(Arrays.stream(manager.getTable(INDEXED_MANSION).pools).toList());
            }
        }));

        LootTableEvents.MODIFY.register(((resourceManager, manager, id, supplier, setter) -> {
            if(LootTables.ABANDONED_MINESHAFT_CHEST.equals(id)) {
                supplier.pools(Arrays.stream(manager.getTable(INDEXED_MINESHAFT).pools).toList());
            }
        }));

        LootTableEvents.MODIFY.register(((resourceManager, manager, id, supplier, setter) -> {
            if(LootTables.SHIPWRECK_TREASURE_CHEST.equals(id)) {
                supplier.pools(Arrays.stream(manager.getTable(INDEXED_SHIPWRECK).pools).toList());
            }
        }));

        LootTableEvents.MODIFY.register(((resourceManager, manager, id, supplier, setter) -> {
            if(LootTables.BURIED_TREASURE_CHEST.equals(id)) {
                supplier.pools(Arrays.stream(manager.getTable(INDEXED_BURIED_TREASURE).pools).toList());
            }
        }));
        ...
        LootTableEvents.MODIFY.register(((resourceManager, manager, id, supplier, setter) -> {
            if(LootTables.UNDERWATER_RUIN_BIG_CHEST.equals(id) ||
                ...
                supplier.pools(Arrays.stream(manager.getTable(INDEXED_WATER_RUIN).pools).toList());
            }
        }));
        ...
        LootTableEvents.MODIFY.register(((resourceManager, manager, id, supplier, setter) -> {
            if(LootTables.JUNGLE_TEMPLE_CHEST.equals(id) ||
                ...
                supplier.pools(Arrays.stream(manager.getTable(INDEXED_TEMPLE).pools).toList());
            }
        }));
